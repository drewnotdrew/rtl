...
def run_cocotb(top: str, deps: List[str], parameters: Dict[str]):
    """
    ...
    if top.endswith(".sv"):
        top = top[:-3]
    srcs = [resolve_module_path(src) for src in ([top] + deps)]

    cocotb_test.simulator.run(
        verilog_sources=srcs,
        toplevel=srcs[0],
        module=f"test_{top}",
        python_search=[get_project_root().joinpath("verif/py")],
